...
func _mat(c:Color)->StandardMaterial3D:
	...
	var m:=StandardMaterial3D.new();m.albedo_color=c;m.roughness=.92;mats[k]=m;return m

func box(n:String,p:Vector3,s:Vector3,c:Color=STONE,parent:Node3D=self)->MeshInstance3D:
	var x:=MeshInstance3D.new();x.name=n
	var mesh:=BoxMesh.new();mesh.size=s;mesh.material=_mat(c);x.mesh=mesh;x.position=p;parent.add_child(x);return x

func _build()->void:
	var visual:=Node3D.new();visual.name="TempleVisual";add_child(visual)
	var collision:=Node3D.new();collision.name="TempleCollision";add_child(collision)
	box("NaveFloor",Vector3(0,-.2,-86),Vector3(18,.4,30),FLOOR,visual)
	box("ApseFloor",Vector3(0,-.18,-104),Vector3(14,.36,10),FLOOR,visual)
	for x in [-7.0,7.0]:
		box("NaveWall",Vector3(x,3.5,-89),Vector3(.8,7,34),STONE,visual)
	for z in [-79.0,-86.0,-93.0,-100.0]:
		box("ColumnL",Vector3(-5.0,2.7,z),Vector3(.85,5.4,.85),STONE,visual)
		box("ColumnR",Vector3(5.0,2.7,z),Vector3(.85,5.4,.85),STONE,visual)
	box("RearWall",Vector3(0,4,-109),Vector3(18,8,.9),STONE,visual)
...
func _build_smith(p:Vector3,parent:Node3D)->void:
	var r:=Node3D.new();r.name="SmithStation";r.position=p;parent.add_child(r)
...
func _build_merchant(p:Vector3,parent:Node3D)->void:
	var r:=Node3D.new();r.name="MerchantStation";r.position=p;parent.add_child(r)
...
func _build_engraver(p:Vector3,parent:Node3D)->void:
	var r:=Node3D.new();r.name="EngraverStation";r.position=p;parent.add_child(r)
	box("RuneTable",Vector3(0,.7,0),Vector3(2.4,1.1,1.4),STONE,r)
	for i in range(3): box("RuneStone",Vector3(-.65+i*.65,1.4,0),Vector3(.32,.32,.32),Color(.19,.15,.23),r)

func _build_keeper(p:Vector3,parent:Node3D)->void:
	var r:=Node3D.new();r.name="Keeper";r.position=p;parent.add_child(r)
...
func _build_weapon_altar(p:Vector3,parent:Node3D)->void:
	var r:=Node3D.new();r.name="ForgottenCovenant";r.position=p;parent.add_child(r)
