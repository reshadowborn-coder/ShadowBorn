...
func _area(name:String,pos:Vector3)->Area3D:
	var a:=Area3D.new();a.name=name;a.position=pos
	var c:=CollisionShape3D.new();var shape:=BoxShape3D.new();shape.size=Vector3(5,2.5,3);c.shape=shape;a.add_child(c);add_child(a);return a

func _gate(pos:Vector3,name:String,script_path:String)->void:
	var a:=_area(name,pos);a.set_script(load(script_path))

func _interact(pos:Vector3,name:String,kind:String)->void:
	var a:=_area(name,pos);a.set_script(load("res://scripts/world/temple_interaction.gd"));a.set("interaction",kind)

func _room(pos:Vector3,name:String,index:int)->void:
	var a:=_area(name,pos);a.set_script(load("res://scripts/world/catacomb_room_trigger.gd"));a.set("room",index)
