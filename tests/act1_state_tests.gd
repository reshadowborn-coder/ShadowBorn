...
func _run()->void:
	...
	var control_pack:=MultiEnemyEncounter.new()
	root.add_child(control_pack)
	control_pack.set_loadout("sword_shield")
	control_pack.set_reduced_motion(true)
	control_pack.set_presentation_timeline_enabled(true)
	control_pack.set_turn_meter_mode_enabled(true)
	_check(control_pack.start(SewerEncounterPlan.enemies(3),false,true),"pack control fixture starts")
	_check(await _wait_for_pack_player_turn(control_pack),"pack control fixture reaches Shadow's natural turn")
	var rat_b_id:=StringName(str((control_pack.enemies[1] as Dictionary).get("id","")))
	_check(control_pack.apply_turn_provoke(&"shadow",rat_b_id,1),"pack Provoke applies through controller API")
	_check(control_pack.selected==1,"Provoke immediately forces Shadow's target to its source")
	control_pack.select_target(0)
	_check(control_pack.selected==1,"manual target switching cannot escape Provoke")
	_check(control_pack.apply_turn_silence(&"shadow",rat_b_id,1),"pack Silence applies through controller API")
	var control_commands:Array[String]=[]
	control_pack.command_committed.connect(func(skill:String):control_commands.append(skill))
	control_pack.shadow_action("A2")
	_check(control_commands.is_empty() and not control_pack.action_locked,"pack Silence blocks A2 without consuming Shadow's turn")
	control_pack.shadow_action("A1")
	_check(control_commands.size()==1 and control_commands[0]=="A1","pack Provoke + Silence still allows A1 against the forced target")

	p.queue_free()
	recovery_progression.queue_free()
	pack.queue_free()
	control_pack.queue_free()
	await process_frame
