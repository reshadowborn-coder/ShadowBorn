...
func _run()->void:
	...
	var platform_runtime:=_read("res://scripts/platform/platform_runtime_service.gd")
	...
	_check(platform_runtime.contains("NOTIFICATION_OS_MEMORY_WARNING"),"iPhone memory warning is centralized in PlatformRuntime")
	_check(game.contains('get_node("/root/PlatformRuntime")'),"Chapter 0 resolves PlatformRuntime explicitly instead of relying on an autoload symbol")
	_check(game.contains("platform_runtime.memory_pressure.connect(_on_platform_memory_pressure)"),"Chapter 0 responds to semantic memory pressure without owning the OS notification")
	_check(not game.contains("NOTIFICATION_OS_MEMORY_WARNING"),"Chapter 0 no longer duplicates platform memory-warning ownership")
