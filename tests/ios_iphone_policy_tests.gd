...
func _run()->void:
	...
	_check(game.contains("PlatformRuntime.memory_pressure.connect(_on_platform_memory_pressure)"),"Chapter 0 responds to semantic memory pressure without owning the OS notification")
